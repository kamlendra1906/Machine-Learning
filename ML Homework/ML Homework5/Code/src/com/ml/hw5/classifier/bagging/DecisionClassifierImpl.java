...
public class DecisionClassifierImpl implements DecisionClassifier, TreeBuilder {
	...
	private void splitNode(TreeNode node, Queue<TreeNode> nodesToBeSplit, Map<Integer, Boolean> featureUsed) throws Exception {
		...
		if(node.isLeaf() || node.depth() == maxDepth || (dataSet.isClassificationTask() && isPureNode(stats)) || stats.getTotalData() <= minDataPerNode) {
			node.setLeaf();
			return;
		}
		...
		if(bestResult == null || (bestResult != null && bestResult.getInfoGain() < minGain)) {
			node.setLeaf();
			return;
		}
